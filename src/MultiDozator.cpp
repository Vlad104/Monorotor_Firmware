...
void MultiDozator::calculate_reverse(float reverse) {
    data_.reverse = reverse;

    float reverse_A = data_.reverse * data_.ratio_A * data_.gear_A;
    float reverse_B = data_.reverse * data_.ratio_B * data_.gear_B;

    dozator_A_->set_reverse(reverse_A);
    dozator_B_->set_reverse(reverse_B);
}
...
void MultiDozator::calculate_all() {
    ...
    calculate_reverse(data_.reverse);
...
void MultiDozator::start() {
    need_reverse_ = (data_.reverse != 0) ? true : false;
    is_run_ = true;
...
void MultiDozator::reverse() {
    ...
    switch (data_.dozator) {
        case '2':
            dozator_A_->setMinPulseWidth(PULSE_WIDTH / 2);
            dozator_B_->setMinPulseWidth(PULSE_WIDTH / 2);
            dozator_A_->start_reverse();
            dozator_B_->start_reverse();
            break;
        case '0':
            dozator_A_->setMinPulseWidth(PULSE_WIDTH);
            dozator_A_->start_reverse();
...
void MultiDozator::continues_start() {
    need_reverse_ = false; // зависит от ТЗ. Возможно условие как в start()
    is_run_ = true;
    switch (data_.dozator) {
        case '2':
            dozator_A_->setMinPulseWidth(PULSE_WIDTH / 2);
